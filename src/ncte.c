...
static inline void block_winch() { change_winch(SIG_BLOCK); }

static inline void unblock_winch() { change_winch(SIG_UNBLOCK); }
...
int main(int argc, char *argv[]) {
	...
	VTermScreenCallbacks screen_cbs = {
	    .damage =
		screen_damage, /* for now we refresh the screen at our own rate based on a timer */
	...
	if (child == 0) {
		...
		if (child_term != NULL) {
			if (setenv("TERM", child_term, 1) != 0)
				err_exit(errno, "error setting environment variable: %s",
					 child_term);
		} else {
			if (unsetenv("TERM") != 0)
