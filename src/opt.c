...
#define OPT_TERM_INDEX 0
	...
	.default_val =
	    NULL, /* when .default_val is NULL, use the value of TERM in current environment */
...
#define OPT_HELP_INDEX 3
	...
	.default_val = NULL,
	.lopt = {OPT_HELP, 0, 0, 'h'}}}; /* ncte_options */
...
#define F1_WIDTH 26
	...
	fprintf(stdout, "\n%-" NCTE_EXPAND_QUOTE(F1_WIDTH) "s%s\n", "  CMD",
		"run CMD with arguments instead of the default");
	fprintf(
	    stdout,
	    "%-" NCTE_EXPAND_QUOTE(F1_WIDTH) "s\tdefault: the value of SHELL in the environment\n",
	    "");
	...
	for (i = 0; i < NCTE_OPTLEN; i++) {
		...
		if (f1_amt < F1_SIZE && ncte_options[i].lopt.val >= 0 &&
		    ncte_options[i].lopt.val < 256)
			f1_amt += snprintf(f1 + f1_amt, F1_SIZE - f1_amt, "|-%c",
					   ncte_options[i].lopt.val);

		if (f1_amt < F1_SIZE && ncte_options[i].lopt.has_arg &&
		    ncte_options[i].usage != NULL)
